...
`01 → 02 → 03 → 04 → 05` 순서입니다. [`src/4_이변모델/08_full_pipeline.py`](../2_준석_초기_이변모델/08_full_pipeline.py)로 만든 C모델 결과가 먼저 필요합니다. `reports/5_이변모델_선정/결과/`가 11·12·14의 산출물이고, `reports/5_이변모델_선정/03_종합보고서.docx`·`reports/5_이변모델_선정/02_배당구간_필터전략.docx`가 각각 15·13의 산출물입니다.
