...
--------------------------------------------------------------------
[로컬 실행용 수정] 원본은 data/race_entries.csv.gz 를 읽지만, 이 프로젝트
로컬 폴더에는 그 파일이 없고 대신 프로젝트 루트의 final.csv 가 동일한
원천 데이터입니다. RACE_ENTRIES 탐색 로직만 아래처럼 대체했고, 나머지
로직(피처 제외 목록·전처리·모델·평가)은 원본 그대로입니다.
--------------------------------------------------------------------
"""
...
# ── 경로 ──────────────────────────────────────────────────
# [수정] data/race_entries.csv.gz 가 있으면 그걸, 없으면 프로젝트 루트의
# final.csv 를 쓴다 (같은 원천 데이터의 다른 파일명/형식).
_ROOT = Path(__file__).resolve().parent
for _ in range(4):
    if (_ROOT / "data" / "race_entries.csv.gz").exists():
        break
    _ROOT = _ROOT.parent
_gz = _ROOT / "data" / "race_entries.csv.gz"

_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent  # src/model_selection_validation -> 프로젝트 루트
RACE_ENTRIES = _gz if _gz.exists() else _PROJECT_ROOT / "final.csv"
