...
| `07_split_data.py` | 8개 버전 CSV를 시간순 fold 컬럼 기준으로 train/valid/test로 물리 분리 | `data/splits/<version>/{train,valid,test}.csv` → `data/전처리_데이터셋/` | 박준석 (junseok) |
...
각 스크립트는 프로젝트 루트의 `race_entries.csv`를 입력으로 사용합니다.
