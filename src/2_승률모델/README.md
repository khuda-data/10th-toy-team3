...
`reports/2_승률모델/01_1차결과.docx`(1차 모델링 결과)를 만든 코드입니다. 로지스틱회귀/랜덤포레스트/XGBoost로 1착 여부를 예측하고, 배당률의 시장확률과 비교합니다.
...
| `05_model_report.py` | 위 결과를 하나의 HTML 보고서로 종합 | `results/report.html` → `reports/2_승률모델/01_1차결과.docx` | 박준석 (junseok) |
| `config.py` | 컬럼 분류·유틸리티 함수 등 파이프라인 공통 설정 모듈 (팀 공용 설정. 각 파이프라인이 독립 실행되도록 같은 파일을 폴더마다 복사해 뒀습니다 — 7곳 모두 내용이 동일합니다) | — | 팀 공용 (원 작성자 미상) |

## 실행 순서

`01 → 02 → 03 → 04 → 05` 순서로 실행합니다. `01_train_model.py`가 만든 `results/models/`는 `src/3_시장대조/방향실험/`의 세 방향(확률 보정·시장 결합·이변 예측) 실험이 그대로 이어받아 사용합니다.
