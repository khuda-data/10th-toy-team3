...
저장소 최상위의 [`data/raw/race_entries.csv.gz`](../../../../../data/raw/race_entries.csv.gz)가 바로 그 파일입니다. 이름만 다릅니다 — 정리 과정에서 "final"이 단계를 뜻하는 것처럼 읽혀 `race_entries`로 바꿨습니다.

저장소 루트에서 실행하세요.

```powershell
Copy-Item "data\raw\race_entries.csv.gz" `
          "src\3_시장대조\시장확률보정\data\raw\final.csv.gz"
```

```bash
cp data/raw/race_entries.csv.gz \
   src/3_시장대조/시장확률보정/data/raw/final.csv.gz
```
...
**대부분의 코드가 멈춥니다.** 저장소 정리 과정에서 재생성 가능한 중간 데이터
(`interim/seoul_entries.csv.gz`, `predictions/*.csv.gz`, `analysis/*.csv.gz` — 합계 18.5MB)를
제외했기 때문에, 이제 이 원본이 파이프라인의 **유일한 출발점**입니다.

| 항목 | 원본 없이 |
|---|---|
| `tests/test_schema.py` | 실패 — 파일 없음 |
| 학습·평가·백테스트 테스트 | 실패 — 중간 데이터 없음 |
| `python -m src.data.build_seoul_interim` | 실행 불가 |
| 정책·계약 검증 테스트 (`test_dataset_policy` 등) | **동작합니다** — `manifests/` 만 읽습니다 |

복원 순서는 [`../README.md`](../README.md)에 있습니다.
