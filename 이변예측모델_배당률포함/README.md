...
- `이변_예측모델_배당률포함_보고서.docx` — 최종 상세 보고서
- `code/` — 위 보고서를 만들어낸 실행 파이프라인 (01 → 10 순서로 실행)
...
| `07_report.py` | 01~06 단계의 결과를 하나의 HTML 보고서(`report.html`)로 종합 | 박준석 (junseok) |
...
각 스크립트는 프로젝트 루트의 `final.csv`(또는 `.gz`)를 입력으로 사용합니다.
